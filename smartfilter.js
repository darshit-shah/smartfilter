...
function smartfilter() {
        ...
        for (var n = 0; n < pivotMap[index].dimensions.length; n++) {
            if (typeof pivotMap[index].dimensions[n] === "string") {
                if (pivotMap[index].dimensions[n] !== "") {
                    query.select.push({
                        field: pivotMap[index].dimensions[n],
                        alias: pivotMap[index].dimensions[n]
                    });
                }
            } else if (Array.isArray(pivotMap[index].dimensions[n].values)) {
                var caseStatement = {
                    field: pivotMap[index].dimensions[n].values[0].key,
                    expression: {
                        cases: [],
                        'default': {
                            value: 'unknown'
                ...
                for (var i = 0; i < pivotMap[index].dimensions[n].values.length; i++) {
                    caseStatement.expression.cases.push({
                        operator: pivotMap[index].dimensions[n].values[i].type,
                        value: pivotMap[index].dimensions[n].values[i].value,
                        out: {
                            value: '"' + pivotMap[index].dimensions[n].values[i].display + '"'
                        }
                    });
                }
                ...
                query.select.push({
                    field: pivotMap[index].dimensions[n].key,
                    alias: pivotMap[index].dimensions[n].alias,
                    encloseField: pivotMap[index].dimensions[n].encloseField
                });
            }
        }
        ...
        if (forceOrderBy == true) {
            ...
            for (var n = 0; n < pivotMap[index].dimensions.length; n++) {
                if (typeof pivotMap[index].dimensions[n] === "string") {
                    ...
                    query.sortby.push({
                        field: pivotMap[index].dimensions[n].alias,
                        order: 'asc'
                    });
                } else {
                    query.sortby.push({
    ...
    function createPivotWhereCondition(index) {
        ...
        for (var i = 0; i < filterList.length; i++) {
            if (filteredDimension[filterList[i]] != null && filteredDimension[filterList[i]].filters != null && filteredDimension[filterList[i]].filters.length > 0 && (index == -1 || (pivotMap[index] && pivotMap[index].dimensions.length === 1 && (pivotMap[index].dimensions[0] === filterList[i] || pivotMap[index].dimensions[0].key === filterList[i])) == false)) {
    ...
    function updateOldResults(data, dimension, cb) {
        for (var i = 0; i < pivotMap.length; i++) {
            ...
            oldResults[queryString] = {
                result: JSON.parse(JSON.stringify(data.data[pivotMap[i].reference]))
            };
        }
        cb(data);
        data = null;
